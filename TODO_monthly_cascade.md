...
- Определить финальную модель источника для monthly run:
  Текущий переходный вариант: orchestrator запускает несколько roots, `salary_primary` берёт source leaf из `allocation_nodes.metadata.source_category_node_id`, остальные prep/reserve ветки находят source nodes по `allocation_node_groups`, а legacy group ids берут из metadata root-ноды.
