...
def extract_cripto(currency: list) -> list:
    ...
    headers = {"Accepts": "application/json", "X-CMC_PRO_API_KEY": COINMARKETCAP_TOKEN}
    ...
    for symbol in currency:
        try:
            ...
            if price_decimal <= 0:
                logging.warning(f"Некорректная цена для {symbol}: {price}")
                continue
            inverted_price = Decimal("1") / price_decimal
            ...
            if timestamp and timestamp.endswith("Z"):
                ...
                logging.warning(
                    f"Некорректное или отсутствующее время обновления для {symbol}: {timestamp}"
                )
                continue
...
def extract(report_date: str, currency: list) -> list:
    try:
        response_json = requests.get(
            f'https://openexchangerates.org/api/historical/{report_date}.json?app_id={OPEN_EXCHANGE_TOKEN}&symbols={",".join(currency)}'
        ).json()
        data = []
        for currency, value in response_json["rates"].items():
            data_dict = {}
            data_dict["timestamp"] = datetime.datetime.utcfromtimestamp(
