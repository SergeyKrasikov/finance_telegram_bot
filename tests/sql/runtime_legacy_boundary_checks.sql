...
BEGIN
    ...
    SELECT pg_get_functiondef('public.insert_spend_v2(bigint,varchar,numeric,varchar,text)'::regprocedure)
    INTO spend_def;

    SELECT pg_get_functiondef('public.insert_revenue_v2(bigint,varchar,numeric,varchar,text)'::regprocedure)
    INTO revenue_def;

    SELECT pg_get_functiondef('public.insert_spend_with_exchange_v2(bigint,varchar,numeric,varchar,text)'::regprocedure)
    INTO spend_fx_def;

    SELECT pg_get_functiondef('public.get_daily_transactions(bigint)'::regprocedure)
    INTO daily_def;

    SELECT pg_get_functiondef('public.get_last_transaction_v2(bigint,integer)'::regprocedure)
    INTO history_def;

    SELECT pg_get_functiondef('public.get_all_balances_v2(bigint,integer)'::regprocedure)
    INTO balances_def;
    ...
    IF POSITION('cash_flow' IN exchange_def) > 0 THEN
        RAISE EXCEPTION 'exchange_v2() should not reference cash_flow';
    END IF;

    IF POSITION('cash_flow' IN spend_def) > 0 THEN
        RAISE EXCEPTION 'insert_spend_v2() should not reference cash_flow';
    END IF;

    IF POSITION('cash_flow' IN revenue_def) > 0 THEN
        RAISE EXCEPTION 'insert_revenue_v2() should not reference cash_flow';
    END IF;

    IF POSITION('cash_flow' IN spend_fx_def) > 0 THEN
        RAISE EXCEPTION 'insert_spend_with_exchange_v2() should not reference cash_flow';
    END IF;
    ...
    IF POSITION('cash_flow' IN history_def) > 0 THEN
        RAISE EXCEPTION 'get_last_transaction_v2() should not reference cash_flow';
    END IF;

    IF POSITION('cash_flow' IN balances_def) > 0 THEN
        RAISE EXCEPTION 'get_all_balances_v2() should not reference cash_flow';
    END IF;
